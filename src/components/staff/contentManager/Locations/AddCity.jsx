...
function AddCity({ label, stateId, setIsAddModalOpen, onSucess }) {
  ...
  const handleSubmit = async () => {
    if (data.cityName.trim() === "") {
      notify(`${label} name is required`, "error");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/admin/masters/states/${stateId}/cities`,
        {
          method: "POST",
          headers: {
