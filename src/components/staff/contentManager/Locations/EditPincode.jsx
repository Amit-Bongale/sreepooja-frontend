...
function EditPincode({ label, setIsEditModalOpen, editData, onSucess }) {
  let [data, setData] = useState(editData);
  let [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    if (data.pincode.trim() === "") {
      notify("pincode is required", "error");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/admin/masters/pincodes/${data.id}`,
        {
          method: "PUT",
          headers: {
