import { Edit, Languages, Plus, Search } from "lucide-react";
import { useEffect, useState } from "react";
import { getData } from "../../../api/Api";
import AddLanguage from "../../../components/staff/contentManager/languages/AddLanguage";
import EditLanguage from "../../../components/staff/contentManager/languages/EditLanguage";
...
function ManageLanguages() {
  ...
  const fetchLanguages = async () => {
    const data = await getData("/admin/masters/languages");
  ...
  const filteredData = languages.filter((lang) =>
      ...
      <div className="overflow-x-auto p-4">
        {filteredData.length > 0 ? (
              ...
              {filteredData.map((data) => (
                <tr key={data.id} className="hover:bg-gray-50/50">
                  <td className="px-6 py-4 text-gray-500 font-mono text-xs">
                    LAN-{data.id.toString().padStart(2, "0")}
                  </td>
          ...
          <div className="flex flex-col items-center min-h-[70vh] justify-center gap-2 py-10">
            <Languages className="size-18 text-brand-500" />
            <h2 className="text-lg font-bold text-gray-900">
              {" "}
              No Language Found
            </h2>
            <p className="text-gray-500 text-center">
              Try adjusting your search or add new Language.
            </p>
            <button
              onClick={() => setIsAddModalOpen(true)}
