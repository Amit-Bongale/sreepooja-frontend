import { Edit, Map, Plus, Search } from "lucide-react";
import { useEffect, useState } from "react";
import { getData } from "../../../api/Api";
import { useParams } from "react-router";
import AddPincode from "../../../components/staff/contentManager/Locations/AddPincode";
import EditPincode from "../../../components/staff/contentManager/Locations/EditPincode";

function ManagePincodes() {
  ...
  const fetchData = async () => {
    const data = await getData(`/admin/masters/cities/${cityId}/pincodes`);
    setData(data);
  };
      ...
      <div className="overflow-x-auto p-4">
        {filteredData.length > 0 ? (
          ...
          <div className="flex flex-col items-center min-h-[70vh] justify-center gap-2 py-10">
            <Map className="size-18 text-brand-500" />
            <h2 className="text-lg font-bold text-gray-900">
              No Pincodes Found
            </h2>
            <p className="text-gray-500 text-center">
              Try adjusting your search or add new Pincodes.
            </p>
            <button
              onClick={() => setIsAddModalOpen(true)}
